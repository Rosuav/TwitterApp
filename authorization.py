...
def get_request_token():
	""" Get a token allowing to request user authorization"""
	oauth = OAuth1(CLIENT_KEY,client_secret = CLIENT_SECRET)
	response = requests.post(REQUEST_TOKEN_URL,auth=oauth)
	print response.text
	""" In the reponse to our request we get a string in response.contents which contains the request token and secret as a query string. We use the parse_qs function from Python's urlparse module to create a dictionary containing the token and the secret"""
